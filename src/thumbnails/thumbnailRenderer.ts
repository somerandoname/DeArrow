...
import { getThumbnailUrl, getVideoThumbnailIncludingUnsubmitted, isActiveThumbnailCacheRequest, isFetchingFromThumbnailCache, queueThumbnailCacheRequest, waitForThumbnailCache } from "../dataFetching";
...
export async function renderThumbnail(videoID: VideoID, width: number,
    ...
    if (dontLocalRender) {
        if (!isCachedThumbnailLoaded(videoID, timestamp) && !isFetchingFromThumbnailCache(videoID, timestamp)) {
            queueThumbnailCacheRequest(videoID, timestamp, undefined, false, true);
        }

        let tries = 0;
        while (isFetchingFromThumbnailCache(videoID, timestamp) && tries < 10) {
    ...
    if (dontLocalRender) {
        if (thumbnailDataCache.getFromCache(videoID)?.thumbnailCachesFailed?.has(timestamp)) {
            dontLocalRender = false;
        } else {
    ...
    return await new Promise(async (resolve, reject) => {
        ...
        let format: Format | string | undefined | null = null;
        if (!reusedVideo) {
            format = await Promise.race([getPlaybackFormats(videoID, width, height), stopCallbackHandler]);
        ...
        const video = createVideo(reusedVideo, typeof format === "string" ? format : (format?.url ?? ""), timestamp);
        const videoCacheObject: ThumbnailVideo = {
            video: video,
            width: typeof format === "object" && format !== null ? format.width : width,
            height: typeof format === "object" && format !== null ? format.height : height,
            rendered: false,
            onReady: [resolve],
            timestamp
        };
...
export async function createThumbnailImageElement(existingElement: HTMLImageElement | null, videoID: VideoID, width: number,
    ...
    const activeRender = activeRenders[videoID] ?? renderThumbnail(videoID, width, height, saveVideo, timestamp, !isUnsubmitted).finally(() => {
